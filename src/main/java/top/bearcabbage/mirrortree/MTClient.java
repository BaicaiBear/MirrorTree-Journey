...
public class MTClient {
    ...
    public static class RestartNotifier {
            ...
            try {
                if (os.contains("win")) {
                    ...
                    Runtime.getRuntime().exec(new String[]{
                            "cmd", "/c", "msg", "*", "MirrorTree 客户端更新完成，请重启游戏。"
                    });
                } else if (os.contains("mac")) {
                    // macOS 使用 AppleScript 弹窗
                    String[] cmd = {
                            "osascript", "-e",
                            "display dialog \"MirrorTree 客户端更新完成，请重启游戏。\" buttons {\"确定\"} default button 1"
                    };
                    Runtime.getRuntime().exec(cmd);
                } else if (os.contains("nix") || os.contains("nux") || os.contains("aix")) {
                    // Linux 使用 zenity 工具弹窗（需要安装zenity）
                    String[] cmd = {
                            "zenity", "--info", "--text=MirrorTree 客户端更新完成，请重启游戏。"
                    };
                    Runtime.getRuntime().exec(cmd);
                } else {
                    System.out.println("[MirrorTree] 未知系统，MirrorTree 客户端更新完成，无法弹出系统通知，请手动重启游戏。");
                }
            } catch (IOException e) {
                LOGGER.error(e.toString());
                System.out.println("[MirrorTree] MirrorTree 客户端更新完成，弹窗通知失败，请手动重启游戏。");
            }
        }
    }
}
